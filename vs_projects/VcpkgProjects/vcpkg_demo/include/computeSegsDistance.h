...
#include<algorithm>
using namespace std;
namespace Segs_Distance_Algorithm {
	...
	void minDistanceOf2Segs(std::vector<Point> segsA, std::vector<Point> segsB, double& maxDistance, double& minDistance) {
		A = segsA.size(); B = segsB.size();
		for (int i = 0; i < A; i++) P[i] = segsA[i];
		for (int i = 0; i < B; i++) Q[i] = segsB[i];
		...
		maxDistance = Max; minDistance = Min;
	}
}