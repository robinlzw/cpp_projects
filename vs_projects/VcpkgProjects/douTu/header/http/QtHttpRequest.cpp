...
QtHttpRequest::QtHttpRequest(const std::string &url) : HttpRequest(url) {
    _originUrl = url;
...
void QtHttpRequest::addFromFile(const std::string &filePath) {
    CURL *curl = _httpCore;
    if (curl != nullptr) {
...
/**
  ...
  * @date     2018/10/16
  */
void QtHttpRequest::setProcessCallback(const std::string& key,
