    ...
    :QFrame(parent)
{
	...
	auto filePath = dirPath + "/" + QString(bytePwdMd5.toHex());	
...
void CellWgt::setCellImage(const QString& image) {
	qDebug() << "CellWgt::setCellImage -------- image = " << image;
    _path = image;
...
void CellWgt::mousePressEvent(QMouseEvent *e) {
	...
	m_md = new QMimeData();
	if (_path.length()) {
		m_md->setImageData(QImage(_path));
	}
...
void CellWgt::mouseReleaseEvent(QMouseEvent *e) {
	qDebug() << "CellWgt::mouseReleaseEvent ----111----";
	if (_press) {
		_press = false;
		qDebug() << "CellWgt::mouseReleaseEvent ----222----";
		if (m_dg) {
			//m_dg->deleteLater();
			delete m_dg;
		}
