...
class hexin_stock_broker_huatai : public stock_broker
{
public:
	hexin_stock_broker_huatai(const std::vector<boost::tuple<std::string, uint16_t>> &server_vec, const std::string& account, const std::string& password1, const std::string& password2, bool wait_response);
	virtual ~hexin_stock_broker_huatai();
...
private:
	...
	int handle_deal_notification(const std::string& resp);
...
private:
	...
	uint8_t m_decrypt_out_buff[0x5a0] = { 0 };//important
