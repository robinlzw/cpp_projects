...
Rectangle {
    ...
    function insertRowData(arrayData) {
        var rowData = new Object()

        //或者这样也行，按照js语法即可
        //var bababa = Object.create(null)

//        rowData["name"]="banana" //方括号 + key的方式设置成员
//        rowData.cost=15  //点+名字的方式设置成员

        for(var i=0;i<myTable.columnCount;i++){
            rowData[tableShowRec.roleNameList[i]] = arrayData[i];
            //console.log("roleNameList[i] = " + roleNameList[i] + ", arrayData[i] = " + arrayData[i])
        }
        sourceModel.insert(sourceModel.count, rowData);
    }
...
//					naviPointsPage.setNewNaviPoint(text, styleData.row, styleData.column)
        ...
        function updateColumn2Table(arrayData){
            ...
            roleNameList = arrayData[2];  // added in 20100114
